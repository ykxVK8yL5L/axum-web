...
impl Video {
    ...
    pub fn pagination(params:&HashMap<String, String>,conn: &Connection) -> QueryResult<String> {
        ...
        let order_column_name= match sort_name.as_str() {
            "id" => "id",
            "name" => "name",
            "title" => "title",
            "size" => "size",
            ...
            _ => "id"
        }; //the order_by column name of  model
        ...
        let total = query(&keyword,order_column_name,sort_dir).select(count(id)).get_result::<i64>(conn)?;
        let data = query(&keyword,order_column_name,sort_dir).limit(length).offset(start).load::<Video>(conn)?;
